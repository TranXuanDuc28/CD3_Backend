...
class ChatAIService {
  ...
  async getDynamicContentFromPosts() {
    try {
      ...
      this.logger.error('Error getting dynamic content from posts', {
        error: error.message
      });
      return [];
    }
  ...
  async getABTestInsights() {
    try {
      ...
      this.logger.error('Error generating A/B test insights', {
        error: error.message
      });
      return [];
    }
  ...
  extractKeywordsFromText(text) {
    ...
    // Travel keywords
    'du lịch', 'tour', 'khách sạn', 'resort', 'nghỉ dưỡng', 'đi chơi', 'tham quan', 'đà nẵng', 'hội an', 'nha trang', 'phú quốc', 'sapa', 'hạ long', 'huế', 'hồ chí minh', 'hà nội',
    // Food keywords
  ...
  extractTopicsFromPost(post) {
    ...
    const foundDestinations = destinations.filter(dest => content.toLowerCase().includes(dest) || title.toLowerCase().includes(dest));
    ...
    const foundActivities = activities.filter(activity => content.toLowerCase().includes(activity) || title.toLowerCase().includes(activity));
  ...
  async generateAIResponse(message, conversationHistory = [], databaseResponses = []) {
    ...
    try {
      ...
      const context = conversationHistory.map(conv => `${conv.message_type === 'received' ? 'User' : 'Assistant'}: ${conv.message_text}`).join('\n');

      // Xây dựng ngữ cảnh từ cơ sở dữ liệu phản hồi
      const dbContext = databaseResponses.map(resp => `Keyword: ${resp.keyword} -> Response: ${resp.response_text}`).join('\n');
      ...
      const dynamicContext = dynamicResponses.length > 0 ? `BÀI VIẾT GẦN ĐÂY (${dynamicResponses.length} bài):
  ${dynamicResponses.slice(0, 5).map(r => `- "${r.post_title}": ${r.response_text}`).join('\n')}` : '';

      // Bối cảnh từ kết quả A/B test
      const abTestContext = abTestResponses.length > 0 ? `KẾT QUẢ A/B TEST (${abTestResponses.length} insights):
  ...
  async refreshDynamicContent() {
    try {
      ...
      this.logger.error('Error refreshing dynamic content', {
        error: error.message
      });
      throw error;
    }
  ...
  async getPostsForDynamicAnalysis(limit = 20, days = 30) {
    try {
      ...
      this.logger.error('Error getting posts for dynamic analysis', {
        error: error.message
      });
      throw error;
    }
